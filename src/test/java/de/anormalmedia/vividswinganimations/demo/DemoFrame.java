...
public class DemoFrame extends JFrame implements Runnable, Moveable {

    private DefaultAnimationRunner defaultAnimationExecutor;

    public DemoFrame() {
	super("Swing Animations");
	// Window translucency in Java 7 is only supported for undecortated
	// windows.
	setUndecorated(true);
	...
	final Rectangle frameBounds = DemoFrame.this.getGraphicsConfiguration()
		.getBounds();

	defaultAnimationExecutor = new DefaultAnimationRunner();
	...
	BackgroundColorAnimation greenBackgroundColorAnimation = new BackgroundColorAnimation(
		contentPane, Color.GREEN);
	greenBackgroundColorAnimation.setDuration(1000);
	defaultAnimationExecutor.addAnimation(greenBackgroundColorAnimation);

	BackgroundColorAnimation redBackgroundColorAnimation = new BackgroundColorAnimation(
		contentPane, Color.RED);
	...
	BackgroundColorAnimation blueBackgroundColorAnimation = new BackgroundColorAnimation(
		contentPane, Color.BLUE);
	...
	Animation windowFadeAnimation = WindowFadeFactory
		.createWindowFadeAnimation(this, 0f);
	...
	windowFadeAnimation.addAnimationListener(new AnimationAdapter() {
		...
		p.x = frameBounds.x + frameBounds.width
			- DemoFrame.this.getWidth();
		DemoFrame.this.setLocation(p);
	    }
	});

	windowFadeAnimation = WindowFadeFactory.createWindowFadeAnimation(this,
		1f);
	...
	LocationAnimation locationXAnimation = new LocationAnimation(this,
		frameBounds.x + frameBounds.width / 2 + 100, -1);
	...
	SizeAnimation collapseHeighthAnimation = new SizeAnimation(this, -1,
		100);
	...
	LocationAnimation locationYAnimation = new LocationAnimation(this, -1,
		250);
	...
	CircleAnimation circleReverseAnimation = new CircleAnimation(this, 200,
		false);
	...
	BackgroundColorAnimation blackBackgroundColorAnimation = new BackgroundColorAnimation(
	...
	ForegroundColorAnimation foregroundColorAnimation = new ForegroundColorAnimation(
		lblGeil, Color.WHITE);
	...
	BackgroundColorRangeAnimation prepareColorRangeAnimation = new BackgroundColorRangeAnimation(
		contentPane, new Color(Color.HSBtoRGB(0f, 1f, 1f)));
	...
	BackgroundColorRangeAnimation backgroundColorRangeAnimation = new BackgroundColorRangeAnimation(
		contentPane, new Color(Color.HSBtoRGB(0.99f, 1f, 1f)));
